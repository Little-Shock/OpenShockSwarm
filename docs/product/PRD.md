...
- 当前仓库已经把 `workspace-file / search-sidecar / external-persistent` 的 memory provider binding 收成 `/v1/memory-center/providers` durable truth，并进入 `/memory` 与 next-run preview；真实外部 durable adapter 仍属后续范围。
...
- 当前仓库已把 `workspace-file / search-sidecar / external-persistent` 三类 provider binding 物化到 memory center，但其中 external durable adapter 仍保持 degraded / 未接入状态，不把设计稿冒充成已落地数据面
- 记忆合并优先级为：
  1. `topic` / `run`
  2. `issue` / `room`
  3. `workspace`

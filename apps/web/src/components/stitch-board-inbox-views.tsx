...
import { getBoardColumns, inboxItems } from "@/lib/mock-data";
...
function inboxKindLabel(kind: (typeof inboxItems)[number]["kind"]) {
  switch (kind) {
    case "approval":
      return "需要批准";
    case "blocked":
  ...
  return (
    <main className="min-h-screen bg-[var(--shock-paper)] px-3 py-3 text-[var(--shock-ink)]">
      <div className="mx-auto grid min-h-[calc(100vh-1.5rem)] max-w-[1720px] overflow-hidden rounded-[8px] border-2 border-[var(--shock-ink)] bg-white shadow-[6px_6px_0_0_var(--shock-ink)] xl:grid-cols-[190px_minmax(0,1fr)]">
        <StitchSidebar active="board" />
        <section className="flex min-h-full flex-col">
          <StitchTopBar tabs={["Dashboard", "Task Board", "Nodes"]} activeTab="Task Board" searchPlaceholder="Search tasks..." />
          <div className="border-b-2 border-[var(--shock-ink)] bg-[var(--shock-yellow)] px-4 py-2">
            <div className="grid items-center gap-3 xl:grid-cols-[220px_160px_1fr_auto_auto]">
              <p className="font-mono text-[10px] uppercase tracking-[0.16em]">Machine Health: 94%</p>
              <p className="font-mono text-[10px] uppercase tracking-[0.16em]">128 agents online</p>
              ...
              <span className="rounded-[4px] border-2 border-[var(--shock-ink)] bg-white px-3 py-1 font-mono text-[10px] uppercase tracking-[0.14em]">Low Latency</span>
              <span className="font-mono text-[10px] uppercase tracking-[0.14em]">sys_time: 14:02:49</span>
            </div>
          </div>
          <div className="flex-1 overflow-x-auto bg-[#f7f7f7] px-4 py-4">
              ...
              {columns.map((column) => (
                <section key={column.title}>
                  <div className="mb-3 flex items-center justify-between">
                    ...
                    <span className="rounded-[4px] border-2 border-[var(--shock-ink)] bg-white px-2 py-1 font-mono text-[9px] uppercase tracking-[0.14em]">
                    ...
                    {column.cards.map((card) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <span className="rounded-[2px] bg-[var(--shock-yellow)] px-1 py-0.5 font-mono text-[9px] uppercase tracking-[0.14em]">
                            {card.key}
                          </span>
                          <span className="font-mono text-[10px]">···</span>
                        </div>
                        <h4 className="mt-3 text-sm font-semibold leading-6">{card.title}</h4>
                        <div className="mt-4 space-y-1 font-mono text-[9px] uppercase tracking-[0.14em] text-[color:rgba(24,20,14,0.58)]">
...
export function StitchInboxView() {
  return (
    <main className="min-h-screen bg-[var(--shock-paper)] px-3 py-3 text-[var(--shock-ink)]">
      <div className="mx-auto grid min-h-[calc(100vh-1.5rem)] max-w-[1720px] overflow-hidden rounded-[8px] border-2 border-[var(--shock-ink)] bg-white shadow-[6px_6px_0_0_var(--shock-ink)] xl:grid-cols-[190px_minmax(0,1fr)]">
        <StitchSidebar active="inbox" />
        <section className="flex min-h-full flex-col">
          <StitchTopBar tabs={["Dashboard", "Inbox", "Systems"]} activeTab="Inbox" searchPlaceholder="Search signals..." />
          <div className="flex-1 overflow-y-auto bg-white px-8 py-8">
            <div className="mx-auto max-w-5xl">
              <p className="inline-flex rounded-[4px] bg-[#ead7ff] px-2 py-1 font-mono text-[9px] uppercase tracking-[0.16em] text-[var(--shock-purple)]">
                Human Intelligence Required
              </p>
              <div className="mt-4 flex items-end justify-between gap-6">
                <div>
                  <h1 className="font-display text-5xl font-bold">Human Inbox</h1>
                  <p className="mt-3 max-w-2xl text-sm leading-6 text-[color:rgba(24,20,14,0.62)]">
                    只显示真正需要你独特判断的事项。这里绕过自动过滤器，只保留高价值的人类干预。
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-6 text-right">
                  <div>
                    <p className="font-display text-3xl font-bold">3</p>
                    <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-[color:rgba(24,20,14,0.48)]">Critical Blocks</p>
                  </div>
                  <div>
                    <p className="font-display text-3xl font-bold">12</p>
                    <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-[color:rgba(24,20,14,0.48)]">Awaiting Replies</p>
                  </div>
                </div>
              </div>
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-2 xl:items-end">
                      <Link
                        href={item.href}
                        className={cn(
                          "inline-flex min-w-[150px] items-center justify-center rounded-[4px] border-2 border-[var(--shock-ink)] px-4 py-3 font-mono text-[10px] uppercase tracking-[0.16em]",
                          item.kind === "approval"
                            ? "bg-[var(--shock-yellow)]"
                            : item.kind === "blocked"
                      ...
                      <button className="inline-flex min-w-[150px] items-center justify-center rounded-[4px] border-2 border-[var(--shock-ink)] bg-white px-4 py-3 font-mono text-[10px] uppercase tracking-[0.16em]">

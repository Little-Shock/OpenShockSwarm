...
const evidenceRoot =
  ...
  (await mkdtemp(path.join(os.tmpdir(), "openshock-tkt42-memory-viewer-")));
...
async function waitForMissing(page, testID) {
  await page.waitForFunction(
...
try {
  ...
  await waitForVisible(page, "memory-preview-item-notes-rooms-room-memory-md");
  await capture(page, screenshotsDir, "initial-memory-viewer");
  ...
  await page.waitForFunction(
    () => document.body.textContent?.includes("memory.forget /") ?? false,
    { timeout: 30_000 }
  );
  await capture(page, screenshotsDir, "memory-forgotten");

  const report = [
    "# Test Report 2026-04-09 Memory Viewer / Correction / Forget Surface",
    "",
    `- Command: \`pnpm test:headed-memory-viewer-correction-forget -- --report ${path.relative(projectRoot, reportPath)}\``,
